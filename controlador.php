...
require("conexion_bd.php"); //es para que incluya el archio de conexion a la BS y servidor
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { //es una condicional, es para ue se espea una peticion POST y no otro tipo
    if (empty($_POST["uname"]) || empty($_POST["psw"])) { //sire para que no haga nada si los campos estan acios
        ...
        $consultasql = $conexion->prepare("SELECT * FROM usuarios WHERE user = ?"); // conexion-> prepare sirve para idnicar que quieroo hacer una consulta
        ...
        $consultasql->bind_param("s", $usuario); //vincula variables php a los marcadores de posiciion de la consulta sql
        $consultasql->execute(); // indica que ejecute la coonsulta preparada antes
        $resultado = $consultasql->get_result(); // recoge el resultado de la consulta
